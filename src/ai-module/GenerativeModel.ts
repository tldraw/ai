...
import { Box, Editor, TLAnyShapeUtilConstructor, useEditor } from 'tldraw'
import {
...
/** @internal */
export type GenerativeAiChange =
	| CreateShapeChange
	| UpdateShapeChange
	| DeleteShapeChange
...
export class GenerativeModel {
	...
	private makeInput(
		prompt: string,
		options?: GenerativeAiGenerateOptions
	): GenerativeAiInput {
		const shapes = options?.shapes ?? this.editor.getCurrentPageShapes()
		const contextBounds =
			options?.contextBounds ?? this.editor.getViewportPageBounds()
		const promptBounds =
			options?.promptBounds ?? this.editor.getViewportPageBounds()
		const shapeTypes = mapObjectMapValues(
			this.editor.shapeUtils,
			(_, shapeUtil) =>
				(shapeUtil?.constructor as TLAnyShapeUtilConstructor).props
		)
		return {
			prompt,
			shapes,
